...
class FFC(nn.Module):
    ...
    def forward(self, convglo):
        aa=convglo.size(1)
        FR_net1 = self.fr_net(convglo)  # 1,256,25,37


        m = torch.fft.fft2(convglo)  # 1,256,25,37,fushu

        m1 = torch.real(m)
        m2 = torch.imag(m)
        m3 = torch.cat([m1, m2], dim=1)  # shishu
        me = self.fr_net1(m3)  # 1,512,25,19

        me = self.CALayer1(me)

        a = me[:, :aa, :, :]
        b = me[:, aa:2*aa, :, :]
        m4 = a + b * 1j  # 1,256,25,19fushu
        # mee = torch.fft.irfft(m4)  # 1,256,25,36
        mee = torch.fft.ifft2(m4)  # fushu
        mee = torch.abs(mee)  # shishu

        out = FR_net1 + convglo + mee  # 1,256,12,18
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.FFC2 = FFC(64, 64)
        self.FFC3 = FFC(32, 32)
        #self.FFC4 = FFC(256, 256)

        self.outc = outconv(32, 3)

    def forward(self, x):
        #img = torch.cat((level, x), 1)
        inx = self.inc(x)#1,32,400,600

        down1 = self.down1(inx)
        conv1 = self.conv1(down1)#1,64,200,300

        down2 = self.down2(conv1)
        conv2= self.conv2(down2)#1,128,100,150

        down3 = self.down3(conv2)
        conv3 = self.conv3(down3)#1,256,50,75

        down4 = self.down4(conv3)
        conv4 = self.conv4(down4)#1,256,25,37
        ...
        # FR_net1 = self.fr_net(convglo)  # 1,256,25,37
        # FR_net1 = self.CALayer(FR_net1)
        #
        # m = torch.fft.fft2(convglo)  # 1,256,25,37,fushu
        # # w=torch.fft.ifftn(m)#1,256,25,36
        # m1 = torch.real(m)
        # m2 = torch.imag(m)
        # m3 = torch.cat([m1, m2], dim=1)  # shishu
        # me = self.fr_net1(m3)  # 1,512,25,19
