...
def conv2d(input, weight, padding=0, sample_wise=False):
    ...
    if sample_wise:
        ...
        temp = pad(input_sw, padding, mode='circular')
        # temp = torch.ones(1, 64, 404, 604)
        # weight_sw = torch.ones(64,3,5,5)
        # out = F.conv2d(temp, weight_sw, groups=1)
        b=pad(input_sw, padding, mode='circular')
        b=b.repeat(1, input_sw.size(0), 1, 1)
        out = F.conv2d(b, weight_sw, groups=input.size(0))
        #out = F.conv2d(input_sw,weight_sw,stride=1, padding=0)
